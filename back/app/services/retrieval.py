...
from app.ai.embeddings import embed_text, get_embedding_metadata
...
def search_similar_chunks(
    ...
    try:
        ...
        distance = ContentChunk.embedding.cosine_distance(query_embedding).label(
            "distance"
        )
        ...
        results = db.execute(
            ...
            .where(ContentChunk.node_id == node_id if node_id is not None else True)
            .order_by(distance)
            .limit(top_k)
        ).all()
